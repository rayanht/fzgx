...
unsigned char *fn_800587A8(const SJCK *source, const char *tag, const char *terminator,
                           SJCK *result);

void *memcpy(void *destination, const void *source, unsigned long size);

int fn_12_2FA90(SfdHandle *handle, int channel, SJ *stream_joint, SfdCallbackObject object);

extern const char lbl_12_rodata_17C0[8];

extern const char lbl_12_rodata_17C8[8];

void fn_12_35180(MwsPlayer *player) {
    SJCK source;
    SJCK result;
    SJCK consumed;
    ...
    source.len = num_data;
    if (fn_800587A8(&source, lbl_12_rodata_17C0, lbl_12_rodata_17C8, &result) == 0) {
        player->tag_data = 0;
        player->tag_size = 0;
        player->tag_state = 1;
        return;
