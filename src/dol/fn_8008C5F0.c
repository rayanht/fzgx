...
DSError fn_80088B44(TRKBuffer *buffer, u32 *data, int count);
...
DSError fn_8008C5F0(u32 firstRegister, u32 lastRegister, TRKBuffer *b, size_t *registersLengthPtr,
                    BOOL read) {
    DSError error;
    u32 count;
    u32 *data;
    ...
    *registersLengthPtr = count * sizeof(u32);
    if (read) {
        error = fn_80088DB4(b, data, count);
    } else {
        error = fn_80088B44(b, data, count);
    }
