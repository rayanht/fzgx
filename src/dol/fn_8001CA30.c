...
void fn_8001C56C(s32 chan, u32 error, OSContext *context);
...
void fn_8001CA30(s32 chan, u32 type) {
    u32 error;
    u32 chanBit;
    chanBit = 0x80000000 >> chan;
    if (lbl_801A697C & chanBit) {
        ...
        if (!(error & (0x0001 | 0x0002 | 0x0008 | 0x0004)) && (type & 0x80000000) &&
            ...
            SITransfer(chan, &lbl_801A64C8, 1, &lbl_8015D0C0[chan], 10, fn_8001C56C, 0);
        } else {
            PADDisable(chan);
        }
    }
