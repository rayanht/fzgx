...
int fn_80044A94(AdxBasicDecoder *d, signed char *input, int length);

void *fn_80044C74(void *, int, int *, int *, int *, int *, int *);
...
int fn_80044A94(AdxBasicDecoder *d, signed char *input, int length) {
    ...
    if (length < 8) {
        ...
        data = fn_80044C74(input, length, &rate, &channels, &bits, &samples, &codec);
        if (data == ((void *)0)) {
            result = -1;
        } else {
    ...
    d->fields.view_format_type.format_type = 4;
    d->fields.view_codec_type.codec_type = codec;
    return data_length;
}
